...
fs::path make_temp_dir()
{
    ...
    std::mt19937_64 rng(142857);
    for (int i = 0; i < 64; ++i)
    {
        const fs::path candidate = base / ("autosage-joule-heating-" + std::to_string(rng()));
        std::error_code ec;
...
{
    return stderr_text.find("JouleHeating solver requires MFEM built with MPI.") != std::string::npos ||
        stderr_text.find("JouleHeating residual norm is non-finite.") != std::string::npos;
}
...
int main(int argc, char **argv)
{
    try
    {
        require(argc >= 2, "Usage: JouleHeatingIntegrationTest <path-to-mfem-driver>");
...
0 1
        ...
        if (exit_status != 0)
        {
            ...
            {
                std::cout << "JouleHeating integration test skipped: " << stderr_text << std::endl;
        ...
        const fs::path metadata_path = run_dir / "joule_heating.json";
        require(fs::exists(metadata_path), "Expected joule_heating.json artifact is missing.");

        const json metadata_json = load_json(metadata_path);
        require(
            metadata_json.value("solver_class", "") == "JouleHeating",
            "Expected solver_class=JouleHeating."
        );
        require(
            metadata_json.value("solver_backend", "") == "backward_euler_staggered",
            "Expected solver_backend=backward_euler_staggered."
        );
        require(
        ...
        require(
            metadata_json.contains("time_steps") && metadata_json["time_steps"].is_number_integer() &&
                metadata_json["time_steps"].get<int>() > 0,
            "Expected positive time_steps in joule_heating.json."
        );
        require(
            metadata_json.contains("thermal_iterations") && metadata_json["thermal_iterations"].is_number_integer() &&
                metadata_json["thermal_iterations"].get<int>() >= 0,
            "Expected non-negative thermal_iterations in joule_heating.json."
        );
        require(
            metadata_json.contains("electric_iterations") &&
                metadata_json["electric_iterations"].is_number_integer() &&
                metadata_json["electric_iterations"].get<int>() >= 0,
            "Expected non-negative electric_iterations in joule_heating.json."
        );
        ...
        std::cout << "JouleHeating integration test passed. Run dir: " << run_dir << std::endl;
    ...
    {
        std::cerr << "JouleHeating integration test failed: " << ex.what() << std::endl;
        return 1;
    }
}
