...
fs::path make_temp_dir()
{
    ...
    for (int i = 0; i < 64; ++i)
    {
        const fs::path candidate = base / ("autosage-surface-pde-" + std::to_string(rng()));
        std::error_code ec;
...
{
    return stderr_text.find("SurfacePDE solver requires MFEM built with MPI.") != std::string::npos;
}
...
int main(int argc, char **argv)
{
    try
    {
        require(argc >= 2, "Usage: SurfacePDEIntegrationTest <path-to-mfem-driver>");
...
0 1
        ...
        if (exit_status != 0)
        {
            ...
            {
                std::cout << "SurfacePDE integration test skipped: " << stderr_text << std::endl;
        ...
        const fs::path metadata_path = run_dir / "surface_pde.json";
        require(fs::exists(metadata_path), "Expected surface_pde.json artifact is missing.");

        const json metadata_json = load_json(metadata_path);
        require(metadata_json.value("solver_class", "") == "SurfacePDE", "Expected solver_class=SurfacePDE.");
        require(metadata_json.value("solver_backend", "") == "pcg_boomeramg", "Expected solver_backend=pcg_boomeramg.");
        require(
        ...
        require(metadata_json.value("is_closed_surface", true) == false, "Expected is_closed_surface=false.");
        require(
            metadata_json.contains("iterations") && metadata_json["iterations"].is_number_integer() &&
                metadata_json["iterations"].get<int>() >= 0,
            "Expected non-negative iterations in surface_pde.json."
        );
        require(
            ...
            "Expected finite residual_norm in surface_pde.json."
        );
        ...
        std::cout << "SurfacePDE integration test passed. Run dir: " << run_dir << std::endl;
    ...
    {
        std::cerr << "SurfacePDE integration test failed: " << ex.what() << std::endl;
        return 1;
    }
}
