...
namespace autosage
{
const char *MagnetostaticsSolver::Name() const
{
    return "Magnetostatics";
}

MagnetostaticsSolver::MagnetostaticsConfig MagnetostaticsSolver::ParseConfig(
    const json &config,
    int space_dimension,
    int max_boundary_attribute) const
{
    ...
    parsed.magnetic_insulation_marker.assign(boundary_slots, 0);
    ...
    for (const auto &bc : config["bcs"])
    {
        ...
        if (type == "magnetic_insulation" || type == "magnetic-insulation" || type == "magneticinsulation")
        {
            parsed.magnetic_insulation_marker[attribute - 1] = 1;
            continue;
        }
        throw std::runtime_error("config.bcs[].type must be magnetic_insulation.");
    }

    if (boundary_slots > 0)
    {
        const bool has_insulation = std::any_of(
            parsed.magnetic_insulation_marker.begin(),
            parsed.magnetic_insulation_marker.end(),
            [](int marker) { return marker != 0; }
        );
        if (!has_insulation)
        {
            throw std::runtime_error("config.bcs must include at least one magnetic_insulation boundary condition.");
        }
    }
...
SolveSummary MagnetostaticsSolver::Run(
    mfem::Mesh &mesh,
    const json &config,
    const SolverExecutionContext &context)
{
    ...
    const MagnetostaticsConfig parsed = ParseConfig(config, space_dimension, max_boundary_attribute);

    mfem::ND_FECollection fec(1, dim);
    mfem::ParFiniteElementSpace fespace(&pmesh, &fec);
    mfem::ParGridFunction magnetic_potential(&fespace);
    ...
    {
        ess_bdr[i] = parsed.magnetic_insulation_marker[i];
    }
    ...
    mfem::Vector X;
    mfem::Vector B;
    lhs.FormLinearSystem(ess_tdof_list, magnetic_potential, rhs, A, X, B);
    ...
    lhs.RecoverFEMSolution(X, rhs, magnetic_potential);
    ...
    paraview.RegisterField("magnetic_potential", &magnetic_potential);
    paraview.SetCycle(0);
    paraview.SetTime(0.0);
    paraview.Save();

    std::ofstream vtk_stub(context.vtk_path);
    vtk_stub << "# magnetic potential written to " << collection_name << ".pvd\n";
    ...
    (void)config;
    (void)context;
    throw std::runtime_error("Magnetostatics solver requires MFEM built with MPI.");
#endif
}
} // namespace autosage
